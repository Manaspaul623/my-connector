...
// error_log("Cron Running", 1, "support@aquaapi.com");
    ...
    if (count($resVal) > 0) {
        foreach ($resVal as $key => $value) {
           ...
           // $nextWeek = time();
            ...
            $lastDay = date('Y-m-d', strtotime(date("Y-m-d") . "-2 days"));
									...
									if (count($recordSet) > 0) {
										...
										foreach ($recordSet as $k => $val) {
											...
											// End of Sales Orders.................
													...
													$st = 'YES';
													$AccountHead = 'PRODUCT';
													SaveErrorDetails($uID, $cloudApp, $AccountHead, 'Error adding Product : ' . $prodName . ' Error Code: ' . $resCode. ' Reason:'.$resMsg, $st);
...
function j_InsertIntoZohoContact($contactDetailsCon, $auth, $userid, $crmTypeIAcc) {
    ...
    if ($flag == FALSE) {
        ...
        $AccountHead = 'CUSTOMER';
        $st = 'YES';
        $flagErr = SaveErrorDetails($userid, $crmTypeIAcc, $AccountHead, 'Error creating contact: ' . $email . ':' . $msg1 . ', Code: ' . $msg, $st);
        /* if ($flagErr > 0) {
...
function j_countProducts($zohoAuthtoken, $access_token1, $bigcommURL, $min_date_created_p, $max_date_created_p, $userid, $crmTypeProdC, $accountType2) {
    ...
    if ($http_code === 200) {
        ...
        $count = $count_arr['count'];
        // Added: Debashish
        // error_log($count, 1, "support@aquaapi.com");
...
function j_createProducts($page, $limit, $zohoAuthtoken, $access_token1, $bigcommURL1, $min_date_created_cp, $max_date_created_cp, $userid, $crmTypeProdCr, $accountType1) {
    ...
    for ($i = 1; $i <= $page; $i++) {
        ...
        // Addedd: Debashish
        // error_log($url, 1, "support@aquaapi.com");
...
function j_InsertOrderToZohoInvoice($accountDetails, $auth, $userid, $crmTypeOrdI) {
    ...
    try {
        ...
        // Added : Debashish
        // error_log($response, 1, "support@aquaapi.com");
