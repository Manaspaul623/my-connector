...
{
    // include 'config.php';
    //$url = SUBSCRIPTION_URL . '/products';
    $url = SUBSCRIPTION_URL . '/plans?product_id='.AQUAAPI_CONNECTOR_PRODUCT_CODE;
...
//    include 'config.php';
    ...
    if($customer_id == 1) {
        $data_arr = array(
            ...
            'redirect_url' => 'https://' . APP_DOMAIN . '/app/subscription/callback.php?user_token=' . $user_token . '&planCode=' . $plan_code
        );
    }
    else{
        $data_arr = array(
...
function updateSubscription()
{
    ...
    $planPrice=$res['subscription']['plan']['price']." [ ".$res['subscription']['plan']['name']." ]";
    if ($res['code'] === 0) {
        include "$_SERVER[DOCUMENT_ROOT]/app/mysql/mysqlconstants.php";
        include "$_SERVER[DOCUMENT_ROOT]/app/mysql/mysqllib.php";
        $updateArr = array(
...
function updateCard()
{
    ...
    $data_arr = array(
        ...
        'redirect_url' => 'https://' . APP_DOMAIN . '/app/subscription/callback.php?user_token=' . $user_token
    );
    $data_string = json_encode($data_arr);
...
function cancelSubscription()
{
    ...
    if ($res['code'] === 0) {
        ...
        $user_token = $_REQUEST['row_id'];

        //user detail collecting
        $cond = " AND id='$user_token'";
        $re = fetch($userSubscription, $cond);

        $user_function = $re[0]['function_name'];
        $user_id = $re[0]['user_id'];
        //delete transection Detail
        $cond_delete = " AND user_id='$user_id' AND crm_type='$user_function'";
        $delete_tran = delete($zohoTransactionDetails,$cond_delete);
        //delete Final Subscription
        $cond_delete = " AND user_id='$user_id' AND subscription_id='$zoho_subscription_id'";
        $delete_final = delete($userSubscriptionFinal,$cond_delete);
        //delete Subscription
        $cond = " AND id='$user_token'";
        $re = delete($userSubscription, $cond);
