                            ...
                            if (($resCode === '2000') || ($resCode === '2001') || ($resCode === '2002')) {
                                ...
                                if ($totalProduct > 0) {
                                    ...
                                    for ($i = 0; $i < $totalProduct; $i++) {
                                        ...
                                        $m_adjusted_price = $orders['items'][$key]['items'][$i]['price_incl_tax'];
                                        $m_quantity = $orders['items'][$key]['items'][$i]['qty_ordered'];
                                        $prodListPrice = $orders['items'][$key]['items'][$i]['base_price'];
                                        $prodUnitPrice = $orders['items'][$key]['items'][$i]['base_price_incl_tax'];
                        ...
                        if ($totalProduct > 0) {
                            ...
                            for ($i = 0; $i < $totalProduct; $i++) {
                                ...
                                if ($queryResult->size != 0) // Product exists
                                {
                                    ...
                                    $priceEntry[0]->UnitPrice = $orders['items'][$key]['items'][$i]['row_total'];;
                                ...
                                $orderItem[0]->Quantity = $orders['items'][$key]['items'][$i]['qty_ordered'];
                                $orderItem[0]->UnitPrice = $orders['items'][$key]['items'][$i]['row_total'];;
...
function tblTransectionDetail($credentials, $crmType, $userData)
{
    ...
    if ($count == 0) {
        ...
        foreach ($fetch as $key => $value) {
            $customer = $value['no_customer_data'] + $credentials['counterOrders'];
            $order = $value['no_order_data'] + $credentials['counterAccounts'];
