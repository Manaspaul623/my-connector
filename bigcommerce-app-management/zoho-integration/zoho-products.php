...
function index($zoho_auth_id, $access_token) {
    ...
    $newElement = array();
    //
    $url = "https://api.bigcommerce.com/stores/t0w5eu/v2/products";
    $http_headres = array(
    ...
    foreach ($arrResult as $value):
        $zoho_account_sku = $value['sku'];
        $zoho_name = $value['name'];
        $flag_zoho = insertDataToZohoAccount($zoho_auth_id, $value);
    ...
    print_r($output);
    echo '<br>';
    //$testflg = insertDataToZohoAccount($zoho_auth_id);
...
function insertDataToZohoAccount($auth, $recordSet) {
    //
    $flag = FALSE;
    $prodCode = $recordSet['sku'];
    $prodName = $recordSet['name'];
    $prodDescription = $recordSet['description'];
    $prodUnitPrice = $recordSet['price'];
    $prodListPrice = $recordSet['sale_price'];
    $prodOrdQty = $recordSet['order_quantity_minimum'];
    try {
        /* $xml = '<?xml version="1.0" encoding="UTF-8"?>
          <Products>
          <row no="1">
          <FL val="Product Id">2698400000001362TK</FL>
        ...
        <Products>
            <row no="1">
                ...
                <FL val="Description">' . $prodDescription . '</FL>
                <FL val="Unit Price">' . $prodUnitPrice . '</FL>
                <FL val="List Price">' . $prodListPrice . '</FL>
                <FL val="Quantity">' . $prodOrdQty . '</FL>
            </row>
        </Products>';
